...
To isolate the contributions of the feature gating mechanism and the auxiliary localization loss, we evaluated models with CBAM alone, models with the mask loss alone, and the full proposed configuration ($P$). For both DenseNet121 and ResNet50, adding only the CBAM module without mask supervision decreased AUROC compared to the baseline $C_0$ (DenseNet121 delta $-0.0014$, 95\% CI $[-0.0042, 0.0016]$; ResNet50 delta $-0.0062$, 95\% CI $[-0.0117, -0.0007]$). Utilizing the mask loss without CBAM gating yielded performance comparable to $C_0$. However, combining both components into the full proposed model ($P$) achieved the highest AUROC across all configurations, demonstrating a strong synergistic effect (Figure~\ref{fig:ablation-auroc-deltas}). The full $P$ model consistently outperformed the isolated components on major threshold-free metrics, including classification F1 scores.

\begin{figure}[!htbp]
\centering
\includegraphics[width=0.72\textwidth,keepaspectratio]{figures/component_ablation_delta_ci_auroc.png}
\caption{Component ablation differences in AUROC for DenseNet121 and ResNet50 (95\% CI). The proposed model ($P$) combining both CBAM and mask loss shows synergistic improvements.}
\label{fig:ablation-auroc-deltas}
\end{figure}
\section{Discussion}
