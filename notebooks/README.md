...
## Component Ablation Battery

The original locked B01-B12 runs already cover:

- C0 baseline: `D-C0`, `R-C0`
- full proposed configuration: `D-P`, `R-P` (`CBAM + mask loss`)

To isolate the two method components, run the 12 additional F notebooks:

1. `KAGGLE_F01_D_CBAM_seed3407.ipynb`
2. `KAGGLE_F02_D_CBAM_seed42.ipynb`
3. `KAGGLE_F03_D_CBAM_seed2024.ipynb`
4. `KAGGLE_F04_D_MASK_seed3407.ipynb`
5. `KAGGLE_F05_D_MASK_seed42.ipynb`
6. `KAGGLE_F06_D_MASK_seed2024.ipynb`
7. `KAGGLE_F07_R_CBAM_seed3407.ipynb`
8. `KAGGLE_F08_R_CBAM_seed42.ipynb`
9. `KAGGLE_F09_R_CBAM_seed2024.ipynb`
10. `KAGGLE_F10_R_MASK_seed3407.ipynb`
11. `KAGGLE_F11_R_MASK_seed42.ipynb`
12. `KAGGLE_F12_R_MASK_seed2024.ipynb`

Each F notebook needs the same Kaggle datasets as the B notebooks:
...
- `CBAM-only`: CBAM feature gating is enabled; mask-guided auxiliary loss is off.
- `mask-loss-only`: an auxiliary spatial-attention head receives mask supervision,
  but the classifier features are not gated by CBAM.
- `CBAM + mask loss`: the existing full proposed P setting.

After all F notebooks finish, create or attach Kaggle datasets containing:

- the original B01-B12 locked output folders
- the new F01-F12 ablation output folders

Then run:

13. `KAGGLE_G01_component_ablation_statistics.ipynb`

G01 writes:

- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_input_validation.csv`
- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_by_run_metrics.csv`
- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_summary_table.csv`
- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_pairwise_delta_ci.csv`
- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_mcnemar_tests.csv`
- `/kaggle/working/locked_outputs/component_ablation_statistics/component_ablation_audit.json`
